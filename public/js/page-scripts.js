...
$(document).ready(function () {
    ...
    function buscarPreco(produtoId, callback) {
        if (produtoId) {
            $.get(apiBase + produtoId, function (res) {
                let preco = null;
                if (typeof res === "object") {
                    preco = res.preco || res.valor ||
                        (res.data && (res.data.preco || res.data.valor)) ||
                        (Array.isArray(res) && res.length > 0 && (res[0].preco || res[0].valor));
                }
                callback(preco);
            }).fail(function (err) {
    ...
    function atualizarSubtotal(rowIndex) {
        const quantidade = parseFloat($("#ctrl-quantidade-row" + rowIndex).val()) || 0;
        const valorUnitario = parseFloat($("#ctrl-valor_unitario-row" + rowIndex).val()) || 0;
        const subtotal = quantidade * valorUnitario;
        $("#ctrl-subtotal-row" + rowIndex).val(subtotal.toFixed(2));
        atualizarTotalGeral();
    }
    // 💰 Atualiza o valor total geral
    function atualizarTotalGeral() {
        let total = 0;
        $("input[id^='ctrl-subtotal-row']").each(function () {
            total += parseFloat($(this).val()) || 0;
        });
        $("#value_valor_total").val(total.toFixed(2));
    }
    // 🎯 Ao mudar o produto, buscar preço
    $(document).on("change", "select[id^='ctrl-produto_id-row']", function () {
        const row = $(this).closest("tr");
        const rowIndex = row.attr("data-row");
        const produtoId = $(this).val();
        buscarPreco(produtoId, function (preco) {
            if (preco) {
                $("#ctrl-valor_unitario-row" + rowIndex).val(parseFloat(preco).toFixed(2));
            }
            atualizarSubtotal(rowIndex);
        });
    });
    // 📥 Ao mudar quantidade, buscar preco se precisar e atualizar subtotal
    $(document).on("change", "input[id^='ctrl-quantidade-row']", function () {
        const row = $(this).closest("tr");
        const rowIndex = row.attr("data-row");
        const valorUnitario = parseFloat($("#ctrl-valor_unitario-row" + rowIndex).val());
        const produtoId = $("#ctrl-produto_id-row" + rowIndex).val();
        if (isNaN(valorUnitario) && produtoId) {
            buscarPreco(produtoId, function (preco) {
                if (preco) {
                    $("#ctrl-valor_unitario-row" + rowIndex).val(parseFloat(preco).toFixed(2));
                }
    ...
    // ✍️ Se editar manualmente o valor unitário, atualiza subtotal também
    $(document).on("change", "input[id^='ctrl-valor_unitario-row']", function () {
        const row = $(this).closest("tr");
        const rowIndex = row.attr("data-row");
        atualizarSubtotal(rowIndex);
    });
